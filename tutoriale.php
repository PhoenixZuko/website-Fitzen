...
// Obține articolele care conțin categoria "Tutoriale" fie în `category`, fie în `categories`
$query = $pdo->prepare("
    SELECT 
        a.*, 
        u.username AS author_name, 
        COALESCE((SELECT AVG(rating) FROM ratings WHERE ratings.article_id = a.id), 0) AS average_rating,
        (SELECT COUNT(rating) FROM ratings WHERE ratings.article_id = a.id) AS total_votes,
        (SELECT COUNT(*) FROM comments WHERE comments.article_id = a.id) AS total_comments
    FROM articles a
    JOIN users u ON a.author_id = u.id
    WHERE a.category = :category OR FIND_IN_SET(:category, a.categories)
    ORDER BY a.created_at DESC
");
$query->execute(['category' => 'Tutoriale']);
...
<div id="main">
    <?php if (empty($articles)): ?>
        <p>Nu există articole în categoria "Tutoriale".</p>
    <?php else: ?>
        <?php foreach ($articles as $article): ?>
            <article class="post">
                ...
                <footer>
                    ...
                    <ul class="stats">
                        <li><a href="#"><?= htmlspecialchars($article['category'] ?? 'General') ?></a></li>
                        <li>
